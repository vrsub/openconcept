...
class WettedAreaNonWingGeom_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        S_wet_geometric = 0.9*inputs['ac|geom|fuselage|S_wet']+2*inputs['ac|geom|vstab|S_ref']+2*inputs['ac|geom|hstab|S_ref']
...
class SkinFriction_JetTransport(ExplicitComponent):
    def initialize(self):
        self.options.declare('C_fe', default = 0.0030, desc='Equivalent skin friction coefficient for civil transport, Roskam')
...
class Cd0_NonWing_JetTransport(Group):
     ...
     def setup(self):
        self.add_subsystem('nonwing_wetted_area',WettedAreaNonWingGeom_JetTransport(),promotes_inputs=["*"],promotes_outputs=["*"])
        self.add_subsystem('nonwing_cd0', SkinFrictionNonWing_JetTransport(), promotes_inputs=['*'], promotes_outputs=['*'])
