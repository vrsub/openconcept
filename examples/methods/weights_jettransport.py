...
class WingWeight_JetTrasport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J): # TO DO
        ...
        J['W_wing','ac|geom|wing|c4sweep'] = -(0.0041*inputs['ac|geom|wing|AR']**(1/2)*inputs['ac|geom|wing|S_ref']**0.7490*np.sin(inputs['ac|geom|wing|c4sweep'])*(inputs['ac|weights|MTOW']*n_ult)**0.5570*(inputs['ac|geom|wing|taper'] + 1)**0.1000)/(inputs['ac|geom|wing|toverc']**0.4000*(np.sin(inputs['ac|geom|wing|c4sweep'])**2 - 1))
        J['W_wing','ac|geom|wing|taper'] = (4.1256e-04*inputs['ac|geom|wing|AR']**(1/2)*inputs['ac|geom|wing|S_ref']**0.7490*(inputs['ac|weights|MTOW']*n_ult)**0.5570)/(inputs['ac|geom|wing|toverc']**0.4000*np.cos(inputs['ac|geom|wing|c4sweep'])*(inputs['ac|geom|wing|taper'] + 1)**0.9000)
        J['W_wing','ac|geom|wing|toverc'] = -(0.0017*inputs['ac|geom|wing|AR']**(1/2)*inputs['ac|geom|wing|S_ref']**0.7490*(inputs['ac|weights|MTOW']*n_ult)**0.5570*(inputs['ac|geom|wing|taper'] + 1)**0.1000)/(inputs['ac|geom|wing|toverc']**1.4000*np.cos(inputs['ac|geom|wing|c4sweep']))
...
class FuselageWeight_JetTransport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        ...
        J['W_fuselage','ac|weights|MTOW'] = (0.1552*k_door*k_lg*inputs['ac|geom|fuselage|length']**0.2500*inputs['ac|aero|LoverD']**0.1000*n_ult*inputs['ac|geom|fuselage|S_wet']**0.3020*((3*np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2))/(inputs['ac|geom|wing|taper'] + 1) + 4)**0.0400)/(inputs['ac|weights|MTOW']*n_ult)**0.5000
        ...
        J['W_fuselage','ac|geom|fuselage|length'] = (0.0776*k_door*k_lg*inputs['ac|aero|LoverD']**0.1000*inputs['ac|geom|fuselage|S_wet']**0.3020*(inputs['ac|weights|MTOW']*n_ult)**(1/2)*((3*np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2))/(inputs['ac|geom|wing|taper'] + 1) + 4)**0.0400)/inputs['ac|geom|fuselage|length']**0.7500 - (0.0372*inputs['ac|geom|wing|c4sweep']*k_door*k_lg*inputs['ac|aero|LoverD']**0.1000*inputs['ac|geom|fuselage|S_wet']**0.3020*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2)*(inputs['ac|weights|MTOW']*n_ult)**(1/2)*(np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])**2 + 1))/(inputs['ac|geom|fuselage|length']**1.7500*((3*np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2))/(inputs['ac|geom|wing|taper'] + 1) + 4)**0.9600*(inputs['ac|geom|wing|taper'] + 1))
        J['W_fuselage','ac|geom|fuselage|S_wet'] = (0.0937*k_door*k_lg*inputs['ac|geom|fuselage|length']**0.2500*inputs['ac|aero|LoverD']**0.1000*(inputs['ac|weights|MTOW']*n_ult)**(1/2)*((3*np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2))/(inputs['ac|geom|wing|taper'] + 1) + 4)**0.0400)/inputs['ac|geom|fuselage|S_wet']**0.6980
        J['W_fuselage','ac|aero|LoverD'] = (0.0310*k_door*k_lg*inputs['ac|geom|fuselage|length']**0.2500*inputs['ac|geom|fuselage|S_wet']**0.3020*(inputs['ac|weights|MTOW']*n_ult)**(1/2)*((3*np.tan(inputs['ac|geom|wing|c4sweep']/inputs['ac|geom|fuselage|length'])*(3*inputs['ac|geom|wing|taper'] + 1)*(inputs['ac|geom|wing|AR']*inputs['ac|geom|wing|S_ref'])**(1/2))/(inputs['ac|geom|wing|taper'] + 1) + 4)**0.0400)/inputs['ac|aero|LoverD']**0.9000
...
class MainLandingGear_JetTransport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        n_ult = self.options['n_ult']
        J['W_mlg','ac|weights|MLW']=(0.0094*inputs['ac|geom|maingear|length']**0.4000*n_ult**0.2500*inputs['ac|geom|maingear|n_wheels']**0.3210*inputs['ac|aero|Vstall_land']**0.1000)/inputs['ac|weights|MLW']**0.1120
        J['W_mlg','ac|geom|maingear|length']=(0.0042*n_ult**0.2500*inputs['ac|geom|maingear|n_wheels']**0.3210*inputs['ac|aero|Vstall_land']**0.1000*inputs['ac|weights|MLW']**0.8880)/inputs['ac|geom|maingear|length']**0.6000
        J['W_mlg','ac|geom|maingear|n_wheels'] = (0.0034*inputs['ac|geom|maingear|length']**0.4000*n_ult**0.2500*inputs['ac|aero|Vstall_land']**0.1000*inputs['ac|weights|MLW']**0.8880)/inputs['ac|geom|maingear|n_wheels']**0.6790
        J['W_mlg','ac|aero|Vstall_land'] = (0.0011*inputs['ac|geom|maingear|length']**0.4000*n_ult**0.2500*inputs['ac|geom|maingear|n_wheels']**0.3210*inputs['ac|weights|MLW']**0.8880)/inputs['ac|aero|Vstall_land']**0.9000
...
class NoseLandingGear_JetTransport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        n_ult = self.options['n_ult']
        J['W_nlg','ac|weights|MLW'] = (0.0207*n_ult**0.2000*inputs['ac|geom|nosegear|n_wheels']**0.4500*inputs['ac|geom|nosegear|length']**(1/2))/inputs['ac|weights|MLW']**0.3540
...
class EngineSystems_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        W_engineSystems_Roskam = 9.3300*(0.0010*inputs['W_engine'])**1.0780 + 0.0260*inputs['ac|propulsion|engine|rating']**(1/2) + 0.0820*inputs['ac|propulsion|engine|rating']**0.6500 + 0.0340*inputs['ac|propulsion|engine|rating']
    ...
    def compute_partials(self, inputs, J):
        ...
        J['W_engineSystems','W_engine'] = 0.0101*(0.0010*inputs['W_engine'])**0.0780
...
if __name__ == "__main__":
    ...
    print(prob['OEW.OEW'])
    data = prob.check_partials(method='cs', compact_print=True, show_only_incorrect=True)
