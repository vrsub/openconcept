...
class WingWeight_JetTrasport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        W_wing_Raymer = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557*(inputs['ac|geom|wing|S_ref'])**(0.649+0.1)*(inputs['ac|geom|wing|AR'])**0.5*(inputs['ac|geom|wing|toverc'])**-0.4*(1+inputs['ac|geom|wing|taper'])**0.1*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12**0.1) #*(inputs['ac|geom|wing|S_ref'])**0.1
    ...
    def compute_partials(self, inputs, J): # TO DO
        ...
        # J['W_wing','ac|weights|MTOW'] = (0.0023*inputs['ac|geom|wing|AR']**(1/2)*n_ult*inputs['ac|geom|wing|S_ref']**0.7490*(inputs['ac|geom|wing|taper'] + 1)**0.1000)/(inputs['ac|geom|wing|toverc']**0.4000*np.cos(inputs['ac|geom|wing|c4sweep'])*(inputs['ac|weights|MTOW']*n_ult)**0.4430)
        J['W_wing','ac|weights|MTOW'] = (0.0051*0.557)*(inputs['ac|weights|MTOW']**(0.557-1))*n_ult**0.557*(inputs['ac|geom|wing|S_ref'])**0.649*(inputs['ac|geom|wing|AR'])**0.5*(inputs['ac|geom|wing|toverc'])**-0.4*(1+inputs['ac|geom|wing|taper'])**0.1*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12*inputs['ac|geom|wing|S_ref'])**0.1
        J['W_wing','ac|geom|wing|S_ref'] = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557*(0.649+0.1)*(inputs['ac|geom|wing|S_ref'])**(0.649+0.1-1)*(inputs['ac|geom|wing|AR'])**0.5*(inputs['ac|geom|wing|toverc'])**-0.4*(1+inputs['ac|geom|wing|taper'])**0.1*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12**0.1)
        J['W_wing','ac|geom|wing|AR'] = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557 * (inputs['ac|geom|wing|S_ref'])**0.649*0.5*(inputs['ac|geom|wing|AR'])**(0.5-1)*(inputs['ac|geom|wing|toverc'])**-0.4*(1+inputs['ac|geom|wing|taper'])**0.1*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12**0.1)*(inputs['ac|geom|wing|S_ref'])**0.1
        J['W_wing','ac|geom|wing|c4sweep'] = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557*(inputs['ac|geom|wing|S_ref'])**(0.649+0.1)*(inputs['ac|geom|wing|AR'])**0.5*(inputs['ac|geom|wing|toverc'])**-0.4*(1+inputs['ac|geom|wing|taper'])**0.1*-1*np.cos(inputs['ac|geom|wing|c4sweep'])**-2*(-1*np.sin(inputs['ac|geom|wing|c4sweep']))*(0.12**0.1)
        J['W_wing','ac|geom|wing|taper'] = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557 * (inputs['ac|geom|wing|S_ref'])**0.649*(inputs['ac|geom|wing|AR'])**0.5*(inputs['ac|geom|wing|toverc'])**-0.4*0.1*(1+inputs['ac|geom|wing|taper'])**(0.1-1)*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12**0.1)*(inputs['ac|geom|wing|S_ref'])**0.1
        J['W_wing','ac|geom|wing|toverc'] = 0.0051*(inputs['ac|weights|MTOW']*n_ult)**0.557 * (inputs['ac|geom|wing|S_ref'])**0.649*(inputs['ac|geom|wing|AR'])**0.5*-0.4*(inputs['ac|geom|wing|toverc'])**(-0.4-1)*(1+inputs['ac|geom|wing|taper'])**0.1*np.cos(inputs['ac|geom|wing|c4sweep'])**-1*(0.12**0.1)*(inputs['ac|geom|wing|S_ref'])**0.1
...
class HstabConst_JetTransport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        J['HstabConst','ac|geom|hstab|S_ref'] = 0.6*13*-0.5*inputs['ac|geom|hstab|S_ref']**-1.5*inputs['ac|geom|hstab|AR']**-0.5
        J['HstabConst','ac|geom|hstab|AR'] = 0.6*13*-0.5*inputs['ac|geom|hstab|S_ref']**-0.5*inputs['ac|geom|hstab|AR']**-1.5
...
class HstabWeight_JetTransport(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('ac|geom|hstab|c4_to_wing_c4', units='m', desc='Tail quarter-chord to wing quarter chord disnp.tance')
    ...
    def compute(self, inputs, outputs):
        ...
        W_hstab_raymer = 0.0379*K_uht*inputs['HstabConst']**-0.25*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*(0.3**0.704)*inputs['ac|geom|hstab|c4_to_wing_c4']**-0.296*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
    ...
    def compute_partials(self, inputs, J):
        ...
        J['W_hstab','ac|weights|MTOW'] = 0.0379*K_uht*inputs['HstabConst']**-0.25*0.639*inputs['ac|weights|MTOW']**(0.639-1)*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*inputs['ac|geom|hstab|c4_to_wing_c4']**-1*(0.3*inputs['ac|geom|hstab|c4_to_wing_c4'])**0.704*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
        J['W_hstab','ac|geom|hstab|S_ref'] = 0.0379*K_uht*inputs['HstabConst']**-0.25*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*0.75*inputs['ac|geom|hstab|S_ref']**(0.75-1)*(0.3**0.704)*inputs['ac|geom|hstab|c4_to_wing_c4']**-0.296*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
        J['W_hstab','ac|geom|hstab|AR'] = 0.0379*K_uht*inputs['HstabConst']**-0.25*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*(0.3**0.704)*inputs['ac|geom|hstab|c4_to_wing_c4']**-0.296*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*0.166*inputs['ac|geom|hstab|AR']**(0.166-1)*(1+(0.2))**0.1
        J['W_hstab','ac|geom|hstab|c4sweep'] = 0.0379*K_uht*inputs['HstabConst']**-0.25*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*inputs['ac|geom|hstab|c4_to_wing_c4']**-1*(0.3*inputs['ac|geom|hstab|c4_to_wing_c4'])**0.704*(np.sin(inputs['ac|geom|hstab|c4sweep']))*np.cos(inputs['ac|geom|hstab|c4sweep'])**(-1-1)*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
        J['W_hstab','ac|geom|hstab|c4_to_wing_c4'] = 0.0379*K_uht*inputs['HstabConst']**-0.25*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*(0.3**0.704)*-0.296*inputs['ac|geom|hstab|c4_to_wing_c4']**(-0.296-1)*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
        J['W_hstab','HstabConst'] = 0.0379*K_uht*-0.25*inputs['HstabConst']**(-0.25-1)*inputs['ac|weights|MTOW']**0.639*n_ult**0.10*inputs['ac|geom|hstab|S_ref']**0.75*(0.3**0.704)*inputs['ac|geom|hstab|c4_to_wing_c4']**-0.296*np.cos(inputs['ac|geom|hstab|c4sweep'])**-1*inputs['ac|geom|hstab|AR']**0.166*(1+(0.2))**0.1
...
class VstabWeight_JetTransport(ExplicitComponent):
    ...
    def compute_partials(self, inputs, J):
        ...
        J['W_vstab','ac|weights|MTOW'] = 0.0026*0.556*inputs['ac|weights|MTOW']**(0.556-1)*n_ult**0.536*inputs['ac|geom|hstab|c4_to_wing_c4']**(-0.5+0.875)*inputs['ac|geom|vstab|S_ref']**0.5*np.cos(inputs['ac|geom|vstab|c4sweep'])**-1*inputs['ac|geom|vstab|AR']**0.35*inputs['ac|geom|vstab|toverc']**-0.5
...
class FuselageConst2_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        Kws_raymer = 0.75*inputs['FuseConst1']*inputs['ac|geom|wing|S_ref']**0.5*inputs['ac|geom|wing|AR']**0.5*np.tan(inputs['ac|geom|wing|c4sweep'])*inputs['ac|geom|fuselage|length']**-1
        outputs['K_ws'] = Kws_raymer
    
    def compute_partials(self, inputs, J):
        J['K_ws','FuseConst1'] = 0.75*inputs['ac|geom|wing|S_ref']**0.5*inputs['ac|geom|wing|AR']**0.5*np.tan(inputs['ac|geom|wing|c4sweep'])*inputs['ac|geom|fuselage|length']**-1
        J['K_ws', 'ac|geom|wing|S_ref'] = 0.75*inputs['FuseConst1']*0.5*inputs['ac|geom|wing|S_ref']**(0.5-1)*inputs['ac|geom|wing|AR']**0.5*np.tan(inputs['ac|geom|wing|c4sweep'])*inputs['ac|geom|fuselage|length']**-1
        J['K_ws', 'ac|geom|wing|AR'] = 0.75*inputs['FuseConst1']*inputs['ac|geom|wing|S_ref']**0.5*0.5*inputs['ac|geom|wing|AR']**(0.5-1)*np.tan(inputs['ac|geom|wing|c4sweep'])*inputs['ac|geom|fuselage|length']**-1
        J['K_ws', 'ac|geom|wing|c4sweep'] = 0.75*inputs['FuseConst1']*inputs['ac|geom|wing|S_ref']**0.5*inputs['ac|geom|wing|AR']**0.5*(1/np.cos(inputs['ac|geom|wing|c4sweep']))**2*inputs['ac|geom|fuselage|length']**-1
        J['K_ws', 'ac|geom|fuselage|length'] = 0.75*inputs['FuseConst1']*inputs['ac|geom|wing|S_ref']**0.5*inputs['ac|geom|wing|AR']**0.5*np.tan(inputs['ac|geom|wing|c4sweep'])*-1*inputs['ac|geom|fuselage|length']**(-1-1)
...
class FuselageWeight_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        W_fuselage_raymer = 0.3280*k_door*k_lg*inputs['ac|weights|MTOW']**0.5*n_ult**0.5*inputs['ac|geom|fuselage|length']**0.25*inputs['ac|geom|fuselage|S_wet']**0.302*(1+inputs['K_ws'])**0.04*inputs['ac|aero|LoverD']**0.10
    ...
    def compute_partials(self, inputs, J):
        ...
        J['W_fuselage','ac|weights|MTOW'] = 0.3280*k_door*k_lg*0.5*inputs['ac|weights|MTOW']**(0.5-1)*n_ult**0.5*inputs['ac|geom|fuselage|length']**0.25*inputs['ac|geom|fuselage|S_wet']**0.302*(1+inputs['K_ws'])**0.04*inputs['ac|aero|LoverD']**0.10
        J['W_fuselage','ac|geom|fuselage|length'] = 0.3280*k_door*k_lg*inputs['ac|weights|MTOW']**0.5*n_ult**0.5*0.25*inputs['ac|geom|fuselage|length']**(0.25-1)*inputs['ac|geom|fuselage|S_wet']**0.302*(1+inputs['K_ws'])**0.04*inputs['ac|aero|LoverD']**0.10
        J['W_fuselage','ac|geom|fuselage|S_wet'] = 0.3280*k_door*k_lg*inputs['ac|weights|MTOW']**0.5*n_ult**0.5*inputs['ac|geom|fuselage|length']**0.25*0.302*inputs['ac|geom|fuselage|S_wet']**(0.302-1)*(1+inputs['K_ws'])**0.04*inputs['ac|aero|LoverD']**0.10
        J['W_fuselage','ac|aero|LoverD'] = 0.3280*k_door*k_lg*inputs['ac|weights|MTOW']**0.5*n_ult**0.5*inputs['ac|geom|fuselage|length']**0.25*inputs['ac|geom|fuselage|S_wet']**0.302*(1+inputs['K_ws'])**0.04*0.10*inputs['ac|aero|LoverD']**(0.10-1)
        J['W_fuselage','K_ws'] = 0.3280*k_door*k_lg*inputs['ac|weights|MTOW']**0.5*n_ult**0.5*inputs['ac|geom|fuselage|length']**0.25*inputs['ac|geom|fuselage|S_wet']**0.302*0.04*(1+inputs['K_ws'])**(0.04-1)*inputs['ac|aero|LoverD']**0.10
...
class MainLandingGear_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        W_maingear_raymer = 0.0106*inputs['ac|weights|MLW']**0.888*n_ult**0.25*inputs['ac|geom|maingear|length']**0.4*inputs['ac|geom|maingear|n_wheels']**0.321*inputs['ac|aero|Vstall_land']**0.1
        outputs['W_mlg'] = W_maingear_raymer

    def compute_partials(self, inputs, J):
        n_ult = self.options['n_ult']
        J['W_mlg','ac|weights|MLW']=0.0106*0.888*inputs['ac|weights|MLW']**(0.888-1)*n_ult**0.25*inputs['ac|geom|maingear|length']**0.4*inputs['ac|geom|maingear|n_wheels']**0.321*inputs['ac|aero|Vstall_land']**0.1
        J['W_mlg','ac|geom|maingear|length']=0.0106*inputs['ac|weights|MLW']**0.888*n_ult**0.25*0.4*inputs['ac|geom|maingear|length']**(0.4-1)*inputs['ac|geom|maingear|n_wheels']**0.321*inputs['ac|aero|Vstall_land']**0.1
        J['W_mlg','ac|geom|maingear|n_wheels'] = 0.0106*inputs['ac|weights|MLW']**0.888*n_ult**0.25*inputs['ac|geom|maingear|length']**0.4*0.321*inputs['ac|geom|maingear|n_wheels']**(0.321-1)*inputs['ac|aero|Vstall_land']**0.1
        J['W_mlg','ac|aero|Vstall_land'] = 0.0106*inputs['ac|weights|MLW']**0.888*n_ult**0.25*inputs['ac|geom|maingear|length']**0.4*inputs['ac|geom|maingear|n_wheels']**0.321*0.1*inputs['ac|aero|Vstall_land']**(0.1-1)
...
class NoseLandingGear_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        n_ult = self.options['n_ult']
        W_nosegear_raymer = 0.032*inputs['ac|weights|MLW']**0.646*n_ult**0.2*inputs['ac|geom|nosegear|length']**0.5*inputs['ac|geom|nosegear|n_wheels']**0.45
        outputs['W_nlg'] = W_nosegear_raymer

    def compute_partials(self, inputs, J):
        n_ult = self.options['n_ult']
        J['W_nlg','ac|weights|MLW'] = 0.032*0.646*inputs['ac|weights|MLW']**(0.646-1)*n_ult**0.2*inputs['ac|geom|nosegear|length']**0.5*inputs['ac|geom|nosegear|n_wheels']**0.45
        J['W_nlg','ac|geom|nosegear|length'] = (0.0160*n_ult**0.2000*inputs['ac|geom|nosegear|n_wheels']**0.4500*inputs['ac|weights|MLW']**0.6460)/inputs['ac|geom|nosegear|length']**0.5000
        J['W_nlg','ac|geom|nosegear|n_wheels'] = (0.0144*n_ult**0.2000*inputs['ac|weights|MLW']**0.6460*inputs['ac|geom|nosegear|length']**(1/2))/inputs['ac|geom|nosegear|n_wheels']**0.5500
...
class EngineSystems_JetTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        W_engineSystems_Roskam = 9.3300*0.0010**1.0780*inputs['W_engine']**1.0780 + 0.0260*inputs['ac|propulsion|engine|rating']**(1/2) + 0.0820*inputs['ac|propulsion|engine|rating']**0.6500 + 0.0340*inputs['ac|propulsion|engine|rating']
        outputs['W_engineSystems'] = W_engineSystems_Roskam
    def compute_partials(self, inputs, J):
        J['W_engineSystems','ac|propulsion|engine|rating'] = 0.0533/inputs['ac|propulsion|engine|rating']**0.3500 + 0.0130/inputs['ac|propulsion|engine|rating']**0.5000 + 0.0340
...
class JetTransportEmptyWeight(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('fuselage_constant1',FuselageConst1_JetTransport(),promotes_inputs=["*"],promotes_outputs=["*"])
        self.add_subsystem('fuselage_constant2',FuselageConst2_JetTransport(),promotes_inputs=["*"],promotes_outputs=["*"])
        ...
        self.add_subsystem('engine_systems', EngineSystems_JetTransport(), promotes_inputs=["*"], promotes_outputs=["*"])
        self.add_subsystem('single_engine_wet',AddSubtractComp(output_name='W_singleEngineWet',input_names=['W_engine','W_engineSystems'], units='lb'),promotes_outputs=['*'],promotes_inputs=["*"])
        self.add_subsystem('total_engines', ElementMultiplyDivideComp(output_name='W_engines_total', input_names=['W_singleEngineWet','n_engines'], input_units=['lb','m/m']), promotes_outputs=['*'], promotes_inputs=['*'])
        self.add_subsystem('structural',AddSubtractComp(output_name='W_structure',input_names=['W_wing','W_hstab','W_vstab','W_fuselage','W_mlg','W_nlg'], units='lb'),promotes_outputs=['*'],promotes_inputs=["*"])
        ...
        self.add_subsystem('totalempty',AddSubtractComp(output_name='OEW',input_names=['W_structure_adjusted', 'W_engines_total'], units='lb'),promotes_outputs=['*'],promotes_inputs=["*"])
