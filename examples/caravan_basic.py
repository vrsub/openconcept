...
def run_caravan_analysis():
    ...
    prob.set_val('climb.fltcond|Utrue',np.ones((num_nodes)) * 40,units='kn')
    prob.set_val('cruise.fltcond|Utrue',np.ones((num_nodes)) * 70,units='kn')
    prob.set_val('descent.fltcond|Utrue',np.ones((num_nodes)) * 60,units='kn')

    # set some airplane-specific values.
    # prob['climb.OEW.structural_fudge'] = 1.67
    prob['climb.throttle'] = np.ones((num_nodes))
    prob['cruise.throttle'] = np.ones((num_nodes))
    prob['descent.throttle'] = np.ones((num_nodes))
